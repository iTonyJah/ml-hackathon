...
## Как исправить ошибку в родительской ветке (hotfix)

Если вы работаете в своей ветке `feature/baseline-research` и заметили опечатку или баг в основной ветке `feature/eda-baseline-analysis`, следуйте этой инструкции:

### 1. Сохранение текущего прогресса
Если у вас есть незавершенные изменения, которые вы еще не готовы коммитить, «спрячьте» их:
```bash
git stash
```

### 2. Исправление в донорской ветке
Переключитесь, исправьте ошибку и отправьте правку в репозиторий:
```bash
# Переходим в ветку с ошибкой
git switch feature/eda-baseline-analysis

# (Здесь вы исправляете ошибку в коде)

# Фиксируем и отправляем исправление
git add .
git commit -m "fix: исправление опечатки в baseline"
git push origin feature/eda-baseline-analysis
```

### 3. Синхронизация своей ветки
Вернитесь в свою ветку и заберите в неё сделанное исправление:
```bash
# Возвращаемся в ветку исследования
git switch feature/baseline-research

# Вливаем исправление из донорской ветки
git merge feature/eda-baseline-analysis

# Возвращаем незавершенную работу (если использовали stash)
git stash pop
```

> **Зачем это нужно:** Так вы сразу помогаете всей команде, исправляя баг в общем коде, и при этом поддерживаете свою ветку в актуальном состоянии.
